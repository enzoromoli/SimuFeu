...
import { cellToLatLng } from '../../domain/geoGrid';

// Palette terrain (index → couleur d'affichage) ALIGNÉE par index sur PALETTE_TERRAIN.
// L'ordre/zonage suit terrainStyle ci-dessous (OSM-carto-like).
export const TERRAIN_PALETTE_HEX = [
...
const PALETTE_RGB = TERRAIN_PALETTE_HEX.map((h) => [
...
export function colorToTerrain(r, g, b) {
  return PALETTE_TERRAIN[nearestPaletteIndex(r, g, b)];
}

// OSM tags → couleur d'affichage + z-order (background dessiné en premier).
export function terrainStyle(tags) {
...
// Construit le raster terrain. Retourne l'URL data (pour l'affichage) ET les pixels
// quantifiés + la bbox (pour l'échantillonnage). Nécessite un canvas (navigateur).
export async function buildTerrainRaster(zone) {
...
export function sampleGridTerrain(raster, geo) {
  ...
  for (const cell of makeGrid(geo.radius).values()) {
    ...
    patches.push({ id: cell.id, terrain: colorToTerrain(data[i], data[i + 1], data[i + 2]) });
  }
  return patches;
}
