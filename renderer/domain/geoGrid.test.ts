...
describe('boundsToGrid', () => {
  it('produit une boîte englobante non dégénérée', () => {
    const geo = boundsToGrid(BOUNDS, 12, 10)
    expect(geo.radiusX).toBe(12)
    expect(geo.radiusY).toBe(10)
...
describe('round-trip cellToLatLng ↔ latLngToCell', () => {
  it('chaque cellule revient sur elle-même', () => {
    const geo = boundsToGrid(BOUNDS, 6, 6)
    for (const cell of makeGrid(6, 6).values()) {
  ...
  it('les centres restent dans la bbox géographique', () => {
    const geo = boundsToGrid(BOUNDS, 8, 8)
    for (const cell of makeGrid(8, 8).values()) {
      const { lat, lng } = cellToLatLng(geo, cell.q, cell.r)
...
describe('positionnement', () => {
  it('la cellule centrale (0,0,0) tombe près du centre de la zone', () => {
    const geo = boundsToGrid(BOUNDS, 12, 12)
    const { lat, lng } = cellToLatLng(geo, 0, 0)
    // Le décalage d'une ligne sur deux (voir gridUtils.makeGrid) rend la grille
    // légèrement asymétrique en x : la cellule centrale n'est pas exactement au
    // centre géographique, mais reste à moins d'une largeur de cellule.
    expect(lat).toBeCloseTo((BOUNDS.north + BOUNDS.south) / 2, 5)
    expect(lng).toBeCloseTo((BOUNDS.east + BOUNDS.west) / 2, 2)
  })
